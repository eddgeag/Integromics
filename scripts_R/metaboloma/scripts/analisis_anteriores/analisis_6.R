...
pmanova <- adonis2(scale(metaboloma.log)~grupo*obesidad,method = "euclidean")
females <- adonis2(scale(metaboloma.log)[grupo!="Male",] ~ mujeres,method = "euclidean")$`Pr(>F)`[1]
pcosMales<- adonis2(scale(metaboloma.log)[grupo!="Female",] ~ herma,method = "euclidean")$`Pr(>F)`[1]
sexo.c <- adonis2(scale(metaboloma.log)[grupo!="PCOS",] ~ sexo.control,method = "euclidean")$`Pr(>F)`[1]
...
p.mujeres<- adonis2(mujeres ~ factor.mujeres,method = "euclidean")$`Pr(>F)`[1]
p.pcos <- adonis2(Pcos~ factor.PCOS,method = "euclidean")$`Pr(>F)`[1]
p.males <- adonis2(hombres~ factor.males,method = "euclidean")$`Pr(>F)`[1]
...
p5<-ggsave(p1,filename = "./12-2-22/scripts_analysis_reunion/scripts_metaboloma2/PCA_total.jpeg")
