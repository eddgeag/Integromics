...
metaboloma.total <- datos$comunes$metaboloma

X <- -t(log2(metaboloma.total/colSums(metaboloma.total)))
...
X.sin.norm <- as.data.frame(log2(t(metaboloma.total)))
# 
...
ggsave(filename = file.path(directorio.comunes,"graficos_boxplots_normalizacion.jpg"),plot = p3)
