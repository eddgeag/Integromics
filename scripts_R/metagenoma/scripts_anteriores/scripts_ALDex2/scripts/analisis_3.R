...
for(i in 1:2){
  ...
  resultado.males <- t(taxon.males.res %>% 
                         group_by(condicion) %>% 
                         summarise(across(everything(), sum)))
  colnames(resultado.males) <- resultado.males[1,]
  resultado.males<-resultado.males[-1,]
  ...
  taxon.control.res <- taxon.control[,bacterias_control]
  taxon.control.res$condicion <- control.condicion
  resultado.control <- t(taxon.control.res %>% 
                           group_by(condicion) %>% 
                           summarise(across(everything(), sum)))
  colnames(resultado.control) <- resultado.control[1,]
  resultado.control<-resultado.control[-1,]
  resultado.control <- as.data.frame(resultado.control)
...
for( i in 1:3){
  ...
  resultado.grupo <- as.data.frame(resultado.grupo)
  resultado.grupo$pvalor <- condicion_grupo[rownames(condicion_grupo) %in% bacterias_grupo,c("wi.ep","we.ep","wi.eBH","we.eBH")]
  if(dim(resultado.grupo)[1]==0){
    
    resultado.grupo <- data.frame(`No Obese`=NA,Obese=NA,pvalor=NA)
...
grupo_function <- function(grupo.interes){
  ...
  if(length(bacterias_grupo)==1){
    
    taxon.grupo.res$condicion <- condiciones
    resultado.grupo.interes_ <- ((taxon.grupo.res %>% 
                                                 group_by(condicion) %>% 
                                                 summarise(across(everything(), sum))))
    colnames(resultado.grupo.interes_) <- c("condicion",bacterias_grupo)
    resultado.grupo.interes_$pvalor <- resultados.grupo.interes[rownames(resultados.grupo.interes) %in% bacterias_grupo,c("wi.ep","we.ep","wi.eBH","we.eBH")]
    resultado.grupo.interes_$grupo.interes <- paste(unique(as.character(grupo[grupo!=grupo.interes])),collapse = " ")
    
    
  }else{
    taxon.grupo.res$condicion <- condiciones
    resultado.grupo.interes_ <- as.data.frame(t(taxon.grupo.res %>% 
                                                 group_by(condicion) %>% 
                                                 summarise(across(everything(), sum))))
    colnames(resultado.grupo.interes_) <- resultado.grupo.interes_[1,]
    resultado.grupo.interes_<-resultado.grupo.interes_[-1,]
    resultado.grupo.interes_ <- as.data.frame(resultado.grupo.interes_)
    resultado.grupo.interes_$pvalor <- resultados.grupo.interes[rownames(resultados.grupo.interes) %in% bacterias_grupo,c("wi.ep","we.ep","wi.eBH","we.eBH")]
    resultado.grupo.interes_$grupo.interes <- paste(unique(as.character(grupo[grupo!=grupo.interes])),collapse = " ")
    
  }
   return(resultado.grupo.interes_)
}
...
write.csv(resultado.sexo,file=file.path(directorio,"sexo_general.csv"))
write.csv(resultado.grupo,file=file.path(directorio,"grupo_general.csv"))
write.csv(resultado.obesidad,file=file.path(directorio,"obesidad_general.csv"))
