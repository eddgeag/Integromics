...
pretratado_metaboloma <-
  function(metabolome,
    ...
    if (common) {
      ...
      met37_log <- (apply(met37_norm, 1, log2))
      if (nrow(met37_log) == nrow(variables_in_bacteria)) {
        met37_log <- t(met37_log)
      }
      metaboloma.single <- (met37_log)
      met37_log <- met37_log - min(met37_log) 
      v <-  (voom(
        (met37_log),
        normalize.method = "none",
        design = design
      ))
      metaboloma.procesado <- v$E / v$weights
      ...
      met37 <- (metaboloma)
      met37_norm <- met37 / rowSums(met37)
      met37_log <- t(apply(met37_norm, 1, log2))
      if (nrow(met37_log) == nrow(general_data)) {
        met37_log <- t(met37_log)
      }
      metaboloma.single <- (met37_log)
      met37_log <- met37_log - min(met37_log)
    ...
    datos.metaboloma <- list(
      ...
      metaboloma.single = t(metaboloma.single),
      metaboloma.doble.none = t(metaboloma.doble.none),
      metaboloma.doble.scale = t(metaboloma.doble.scale),
      metaboloma.doble.quantile = t(metaboloma.doble.quantile),
      metaboloma.doble.cyclicloess = t(metaboloma.doble.cyclicloess),
      metaboloma.single.none = t(metaboloma.single.none),
      metaboloma.single.scale = t(metaboloma.single.scale),
      metaboloma.single.quantile = t(metaboloma.single.quantile),
      metaboloma.single.cyclicloess = t(metaboloma.single.cyclicloess)
    )
...
post_procesado_aldex.t <- function(genera.abs,
  ...
  if (condiciones == "obesidad") {
    ...
    conds <- interaction(sexo., obesidad.)
  ...
  genus.aldex <- mean_aldex(genus.aldex)
  phylum.aldex <- mean_aldex(phylum.aldex)
...
voom_microbiota_procesado <-
  function(microbiota,
           variables_in_bacteria,
           diseno) {
    n <- nrow(microbiota)
    p <- ncol(microbiota)
    nn <- nrow(variables_in_bacteria)
    obesidad <- variables_in_bacteria$OBESE
    grupo <- variables_in_bacteria$GROUP
    ...
    micro <- (microbiota)
    micro_norm <- micro / rowSums(micro)
    micro_norm <- micro_norm-min(micro_norm)+0.5
    micro_log <- t(apply(micro_norm, 1, function(x)
      log2(x + 0.5)))
    if (nrow(micro_log) == nn) {
      micro_log <- t(micro_log)
    }
    if(any(microbiota<0)){
      microbiota.min <- microbiota - min(microbiota) 
      
    }else{
      microbiota.min <- microbiota
    }
    
    micro_log[is.na(micro_log)] <- 0
    if(any(micro_log<0)){
      micro_log <- micro_log - min(micro_log) 
    }
    
    microbiota.single <- (micro_log)
    ...
    v <- voom(micro_log,
              normalize.method = "none",
              design = diseno)
    microbiota.procesado <- v$E / v$weights
    microbiota.doble.none <- microbiota.procesado
    v <- voom(micro_log,
              normalize.method = "scale",
              design = diseno)
    microbiota.procesado <- v$E / v$weights
    microbiota.doble.scale <- microbiota.procesado
    v <- voom(micro_log,
              normalize.method = "quantile",
              design = diseno)
    microbiota.procesado <- v$E / v$weights
    microbiota.doble.quantile <- microbiota.procesado
    
    v <- voom(micro_log,
              normalize.method = "cyclicloess",
              design = diseno)
    microbiota.procesado <- v$E / v$weights
    microbiota.doble.cyclicloess <- microbiota.procesado
    
    v <- voom(microbiota.min,
              normalize.method = "none",
              design = diseno)
    microbiota.procesado <- v$E / v$weights
    microbiota.single.none <- microbiota.procesado
    ...
    v <- voom(microbiota.min,
              ...
              design = diseno)
    microbiota.procesado <- v$E / v$weights
    microbiota.single.scale <- microbiota.procesado
    ...
    v <- voom(microbiota.min,
              ...
              design = diseno)
    microbiota.procesado <- v$E / v$weights
    microbiota.single.quantile <- microbiota.procesado
    
    v <- voom(microbiota.min,
              normalize.method = "cyclicloess",
              design = diseno)
    microbiota.procesado <- v$E / v$weights
    microbiota.single.cyclicloess <- microbiota.procesado
    
    datos.microbiota <- list(
      microbiota.crudo = as.data.frame(t(microbiota)),
      microbiota.single = as.data.frame(t(microbiota.single)),
      microbiota.doble.none = as.data.frame(t(microbiota.doble.none)),
      microbiota.doble.scale = as.data.frame(t(microbiota.doble.scale)),
      microbiota.doble.quantile = as.data.frame(t(microbiota.doble.quantile)),
      microbiota.doble.cyclicloess = as.data.frame(t(microbiota.doble.cyclicloess)),
      microbiota.single.none = as.data.frame(t(microbiota.single.none)),
      microbiota.single.scale = as.data.frame(t(microbiota.single.scale)),
      microbiota.single.quantile = as.data.frame(t(microbiota.single.quantile)),
      microbiota.single.cyclicloess = as.data.frame(t(microbiota.single.cyclicloess))
    )
    
    return(datos.microbiota)
  }
...
read_integromics <- function(ruta, condiciones_) {
  ...
  disenos.comunes <- list(
    obesidad = model.matrix(~ obesidad.d),
    grupo = model.matrix(~ grupo.d),
    interaccion = model.matrix(~ grupo.d * obesidad.d),
    sexo = model.matrix(~ sexo.d),
    none = NULL
  )
  disenos.totales <- list(
    obesidad = model.matrix(~ general_data$OBESE),
    grupo = model.matrix(~ general_data$GROUP),
    interaccion = model.matrix(~ general_data$GROUP * general_data$OBESE),
    sexo = model.matrix(~ general_data$SEX),
    none = NULL
  )
  for (diseno in 1:5) {
    metaboloma.comun[[diseno]] <- pretratado_metaboloma(
      metabolome = metabolome,
      variables_in_bacteria = variables_in_bacteria,
      general_data = general_data,
      common = T,
      design = disenos.comunes[[diseno]]
    )
    metaboloma.total[[diseno]] <- pretratado_metaboloma(
      metabolome = metabolome,
      variables_in_bacteria = variables_in_bacteria,
      general_data = general_data,
      common = F,
      design = disenos.totales[[diseno]]
    )
    
  }
  names(metaboloma.comun) <- names(disenos.totales)
  names(metaboloma.total) <- names(disenos.totales)
  
  voom_bayes.genero <- vector(mode = "list", length = 5)
  voom_bayes.filo <- vector(mode = "list", length = 5)
  voom.aldex.filo <- vector(mode = "list", length = 5)
  voom.aldex.genero <- vector(mode = "list", length = 5)
  voom.aldex.filo.filt <- vector(mode = "list", length = 5)
  voom.aldex.genero.filt <- vector(mode = "list", length = 5)
  
  
  for (i in 1:5) {
    diseno_i <- disenos.comunes[[i]]
    voom_bayes.genero[[i]] <-
      voom_microbiota_procesado(
        microbiota = microbiota$bayes.clr$genero.clr,
        variables_in_bacteria,
        diseno = diseno_i
      )
    voom_bayes.filo[[i]] <-
      voom_microbiota_procesado(
        microbiota = microbiota$bayes.clr$filo.clr,
        variables_in_bacteria,
        diseno = diseno_i
      )
    voom.aldex.filo[[i]] <-
      voom_microbiota_procesado(
        microbiota = microbiota$aldex$filo.aldex,
        variables_in_bacteria = variables_in_bacteria,
        diseno = diseno_i
      )
    voom.aldex.genero[[i]] <-
      voom_microbiota_procesado(
        microbiota = microbiota$aldex$genero.aldex,
        variables_in_bacteria = variables_in_bacteria,
        diseno = diseno_i
      )
    voom.aldex.filo.filt[[i]] <-
      voom_microbiota_procesado(
        microbiota = microbiota$aldex.filt$filo.aldex,
        variables_in_bacteria = variables_in_bacteria,
        diseno = diseno_i
      )
    voom.aldex.genero.filt[[i]] <-
      voom_microbiota_procesado(
        microbiota = microbiota$aldex.filt$genero.aldex,
        variables_in_bacteria = variables_in_bacteria,
        diseno = diseno_i
      )
  }
  
  names(voom_bayes.genero) <- names(disenos.comunes)
  names(voom_bayes.filo) <- names(disenos.comunes)
  names(voom.aldex.filo) <- names(disenos.comunes)
  names(voom.aldex.genero) <- names(disenos.comunes)
  names(voom.aldex.filo.filt) <- names(disenos.comunes)
  names(voom.aldex.genero.filt) <- names(disenos.comunes)
  
  microbiota$voom.aldex.filo <- voom.aldex.filo
  microbiota$voom.aldex.genero <- voom.aldex.genero
  microbiota$voom_bayes_genero <- voom_bayes.genero
  microbiota$voom_bayes_filo <- voom_bayes.filo
  microbiota$voom.aldex.filo.filt <- (voom.aldex.filo.filt)
  microbiota$voom.aldex.genero.filt <- (voom.aldex.genero.filt)
...
for (cond in conds) {
  ...
  if (!dir.exists("../../datos/resultados_analisis_datos_procesados_1000_mean_0_2")) {
    dir.create("../../datos/resultados_analisis_datos_procesados_1000_mean_0_2")
  }
  
  saveRDS(datos.integromics,
          file.path(
            paste0(
              "../../datos/resultados_analisis_datos_procesados_1000_mean_0_2/resultados_analisis_datos_procesados_1000_mean_0_2",
              "_",
              cond,
              "_",
              ".rds"
