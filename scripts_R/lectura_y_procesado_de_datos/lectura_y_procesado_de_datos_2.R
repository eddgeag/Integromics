...
pretratado_metaboloma <-
  function(metabolome,
    ...
    if (common) {
      ...
      met37_log <- met37_log - min(met37_log) + 0.5
      v <-  (voom(
        (met37_log),
...
voom_microbiota_procesado <-
  function(microbiota,
    ...
    microbiota.min <- microbiota - min(microbiota) + 0.5
    
    micro_log[is.na(micro_log)] <- 0
    micro_log <- micro_log - min(micro_log) + 0.5
...
for (cond in conds) {
  ...
  if (!dir.exists("../../datos/resultados_analisis_datos_procesados_1000_mean")) {
    dir.create("../../datos/resultados_analisis_datos_procesados_1000_mean")
  }
  
  saveRDS(datos.integromics,
          file.path(
            paste0(
              "../../datos/resultados_analisis_datos_procesados_1000_mean/resultados_analisis_datos_procesados_1000_mean",
              "_",
              cond,
              "_",
              ".rds"
